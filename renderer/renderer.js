...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==================== Voice Commands for Specific Items ====================
  document.getElementById('voice-open-folder').addEventListener('click', async () => {
    const folder = await window.electronAPI.showPrompt('Nombre de la carpeta a abrir:');
    if (!folder) return;
    const ok = openFolderByName(folder);
    showToast(ok ? `Se encontró y abrió “${folder}”` : `No se encontró la carpeta “${folder}”`, ok);
  });

  document.getElementById('voice-open-file').addEventListener('click', async () => {
    const file = await window.electronAPI.showPrompt('Nombre del archivo a abrir:');
    if (!file) return;
    const ok = openFileByName(file);
    showToast(ok ? `Se encontró y abrió “${file}”` : `No se encontró el archivo “${file}”`, ok);
  });
